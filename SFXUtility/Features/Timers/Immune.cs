...
/*
 ...
 Immune.cs is part of SFXUtility.
...
namespace SFXUtility.Features.Timers
{
    ...
    internal class Immune : Base
    {
        ...
        public Immune(IContainer container) : base(container)
        {
        ...
        private void OnDraw(EventArgs args)
        {
            try
            {
                ...
                foreach (var iHero in _immuneHero.ToList().Where(iHero => iHero.TimeUsed != -1))
                {
                    Draw.TextCentered(
                        ...
                        foreach (
                            var buff in
                                hero.Buffs.Where(
                                    buff =>
                                        buff.IsActive &&
                                        (buff.Type == BuffType.Invulnerability ||
                                         _immuneBuffs.Contains(buff.Name, StringComparison.OrdinalIgnoreCase))))
                        {
                            Draw.TextCentered(
            ...
            try
            {
                ...
                foreach (var hero in ObjectManager.Get<Obj_AI_Hero>())
                {
                    if (hero.IsValid && (hero.IsAlly && Menu.Item(Name + "ShowAlly").GetValue<bool>() ||
                                         hero.IsEnemy && Menu.Item(Name + "ShowEnemy").GetValue<bool>()))
                    {
                        foreach (var iStruct in _immuneStructs)
                        {
